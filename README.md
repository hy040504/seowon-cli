...
이 저장소 GUI는 조회 전용 메뉴(로그인·할 일·과제·이러닝·현황)를 **스마트홈 태블릿**처럼 파랑·핑크 타일과 흰 선 아이콘으로 보여 줍니다.
...
메뉴 구성은 조회 전용(로그인·지금 할 것·과제·이러닝·현황·설정·정보)이고, 겉모습은 스마트홈 태블릿처럼 **연한 라벤더 바탕, 흰 둥근 카드, 파랑/핑크 타일, 흰 선 아이콘**입니다.  
...
```text
  (선 아이콘 레일)                 Hello 홍길동!
   ...
   문서 과제                       [할 일] [과제] [이러닝] [현황]
   모니터 이러닝                   (원) 조회          Shortcuts
   막대 현황                       조회 전용 안내      Profile
   톱니 설정
   i   정보
  [홍] 홍길동
       데모
...
| 홈 | 인사, 숫자 칸, 색 타일, 원형 조회, Shortcuts / Profile |
...
- 로그인 뒤에는 홈(`Hello 이름!`)이 열립니다. 분홍 원 바로가기와 파랑/핑크 타일로 과제·이러닝·현황에 갑니다.
...
- 로그인 뒤 첫 화면은 홈(`Hello 이름!`, 숫자 칸, 타일, 원형 조회, Shortcuts).
